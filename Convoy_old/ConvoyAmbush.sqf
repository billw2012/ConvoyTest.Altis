...
diag_log format ["[ConvoyRestart %1] starting convoy ambush defense", _ConvoyID];
...
{ 	
    ...
    _all_enemies = [];
	for "_i" from 0 to (count _arm_groups) - 1 step 1 do
	{
	    _group  = _arm_groups select _i;
	    _units  = units _group;
		{	
		    _enemies 	= [];
		    _unit 	 	= _x;
			...
			if (!(_Enemy_Eval isEqualTo []) && ((_vehicle != _unit) && ((driver _vehicle) == _unit) && _d == 3)) then
			{ 
...
diag_log format ["[ConvoyRestart %1] ending convoy ambush defense", _ConvoyID];
