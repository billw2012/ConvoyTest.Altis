...
private _DEBUGING_UNSTUCK = true;
...
if (_type isEqualTo 'LAND') exitWith {
	if (((vectorUp _vehicle) select 2) < 0.1) then {
		if _DEBUGING_UNSTUCK then {
			systemChat format ["Uprighting %1", _vehicle];
		};
		...
		if ((_vehicle getVariable ['QS_AI_V_stuckCheck',[]]) isEqualTo []) then {
			...
			if (diag_tickTime > ((_vehicle getVariable ['QS_AI_V_stuckCheck',[-1,[0,0,0],-1]]) select 0)) then {
				if ((_vehicle distance2D ((_vehicle getVariable ['QS_AI_V_stuckCheck',[-1,[0,0,0],-1]]) select 1)) < 5) then {
					if _DEBUGING_UNSTUCK then {
						systemChat format ["Detected stuck %1", _vehicle];
					};
					if (((_vehicle getVariable ['QS_AI_V_stuckCheck',[-1,[0,0,0],-1]]) select 2) > 2) then {
						if (_DEBUGING_UNSTUCK or (allPlayers inAreaArray [(getPosATL _vehicle),300,300,0,FALSE]) isEqualTo []) then {
							if _DEBUGING_UNSTUCK then {
								systemChat format ["Unstucking %1 to nearest road", _vehicle];
							};
							...
							_nearestRoad = [((_vehicle getVariable ['QS_AI_V_stuckCheck',[-1,[0,0,0],-1]]) select 1),125] call BIS_fnc_nearestRoad;
