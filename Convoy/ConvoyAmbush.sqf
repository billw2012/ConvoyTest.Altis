...
diag_log format ["[ConvoyAmbush %1] Starting convoy ambush response", _ConvoyID];

while {!_stop_search} do 
{ 	
	if(!_enemy_found && !_stop_search) then {
		diag_log format ["[ConvoyAmbush %1] No enemy found, waiting for a bit before stopping search", _ConvoyID];
		sleep random [15, 25, 40];_stop_search = true;
	};
    _enemy_found = false;
    _Enemy_Eval = [];
		...
		{	
			...
			{ 
				...
				if([objNull, "IFIRE"] checkVisibility [eyePos gunner _vehicle, eyePos _random_enemy] < 0.6) then
				{
					...
					diag_log format ["[ConvoyAmbush %1] Assigning %2 to attack %3", _ConvoyID, _group, _random_enemy];
...

	{
		if(isnull (gunner _x) || !alive (gunner _x)) then
		{
			...
			if(!isnull (Commander _x) && alive (Commander _x)) then {
				diag_log format ["[ConvoyAmbush %1] %2 gunner null or dead, assigning commander", _ConvoyID, _x];
				(Commander _x) assignAsGunner _x;
			};
			if(!isnull (Driver _x) && alive (Driver _x)) then {
				diag_log format ["[ConvoyAmbush %1] %2 gunner null or dead, assigning driver", _ConvoyID, _x];
				(Driver _x) assignAsGunner _x;
			};
			_x limitSpeed 10000;
		};
...
diag_log format ["[ConvoyAmbush %1] Ambush response over, preparing to restart", _ConvoyID];
{if (alive _x && canMove _x && (side _x == _side || !(alive driver _x))) then {
	_aliveConvoy pushBack _x;
	diag_log format ["[ConvoyAmbush %1] %2 can move and has a driver", _ConvoyID, _x];
